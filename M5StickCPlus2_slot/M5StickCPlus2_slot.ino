...
//
// loop()//main function, this function because you need to refresh the slot machine LCD 
// at all times so there can not be a delay similar to delay_ms
...
void loop() {
  ...
  if (state == SLOTS_STOP) {	// The slot machine is in a stopped state, initializing the relevant 
	  			//data to bring the slot machine back to its initial state.
    int symbol = -1;
    bool stopAll = true;
    for (int i = 0; i < SLOT_COUNT; i++) {
  ...
  if (state == SLOTS_FLUSH) {//When all columns are stopped
    if (tick >= flushTick + FLUSH_DELAY) {
      flushTick = tick;
      for (int i = 0; i < SLOT_COUNT; i++) {
        slots[i].flush((flushCount & 1) ? TFT_WHITE : TFT_BLUE);//If all columns have the same icon, the background 
	      							//will be blue otherwise unchanged.
      }
  ...
  for (int i = 0; i < SLOT_COUNT; i++) {//If all columns have the same icon, 
	  				//the background will be blue otherwise unchanged.
    if (slots[i].update()) {
      slots[i].draw();
    }
  }
